...
describe('Progress', () => {
    ...
    const later = new Time(1672956000000); // 23:00:00
    const percent = new Percent(50); // 30 minutes
    const progress = new Progress(start, now, percent);

    test('Progress.copy()', () => {
        expect(progress.copy(percent, now).start().toString()).toBe('22:00:00');
        expect(progress.copy(percent, now).end().toString()).toBe("23:00:00");
        expect(progress.copy(percent, later).start().toString()).toBe('22:00:00');
        expect(progress.copy(percent, later).end().toString()).toBe("00:00:00");
        expect(progress.copy(new Percent(25), now).start().toString()).toBe('22:00:00');
        expect(progress.copy(new Percent(25), now).end().toString()).toBe('00:00:00');
    });
